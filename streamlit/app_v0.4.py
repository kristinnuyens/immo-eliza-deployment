...
st.markdown("""
<style>
/* Remove default Streamlit padding to allow full-width banner */
...
/* Section backgrounds */
.required-section { background-color: #FFF9E6; padding: 10px; border-radius: 10px; }
.optional-section { background-color: #F5F5F5; padding: 10px; border-radius: 10px; }

/* Header banner radius fix on small screens */
@media (max-width: 800px) { .header-banner { border-radius: 0; } }
</style>
...
<div class="header-banner" style="
    ...
    border-radius: 10px;
">
    <h1 style="margin:0;">🏠 Immo Eliza - Property Price Predictor</h1>
    <p style="margin:6px 0 0 0;">Fill in the details below:</p>
</div>
""", unsafe_allow_html=True)
...
with st.container():
    ...
    with col2:
        construction_year = st.number_input("Construction Year *", min_value=1750, max_value=2024, value=2000)
    ...
    with col3:
        total_area_sqm = st.number_input("Internal Area (sqm) *", min_value=3, max_value=88140, value=100)
        surface_land_sqm = st.number_input("Land Area (sqm) *", min_value=0, max_value=950774, value=300)
...
with st.container():
    ...
    with col1:
        ...
        terrace_sqm = st.number_input("Terrace Area (sqm, approx.)", min_value=0, max_value=3466, value=10) if fl_terrace else 0
        fl_garden = st.checkbox("🌳 Garden")
        garden_sqm = st.number_input("Garden Area (sqm, approx.)", min_value=0, max_value=150000, value=50) if fl_garden else 0

    with col2:
        fl_floodzone = st.checkbox("🌊 Flood Zone")
        heating_type_list = ["Wood","Pellet","FuelOil","Gas","Electric","Solar","Carbon","MISSING"]
        heating_type = st.selectbox("Heating Type", heating_type_list)
